...
const QuotesList = ({ saveQuote, handleUnsave }) => {
  ...
  return (
    <div className='w-11/12 pb-10 h-[80vh]  bg-pink-200 my-2 overflow-auto scrollbar-track-gray-100  rounded-3xl flex  flex-col gap-14'>
      <h1 className='text-4xl font-semibold bg-emerald-200 w-1/3 h-28 rounded-ee-full flex items-center pl-10'>YOUR QUOTES</h1>
      {saveQuote.length
      ?
      <ul className='flex flex-col items-center w-full gap-6 '>
      {saveQuote?.map((item)=>(
        <li key={item?.id} className='bg-pink-50 w-1/2 p-4 shadow-xl h-[30vh] flex justify-between even:mr-64 odd:ml-64 flex-col'>

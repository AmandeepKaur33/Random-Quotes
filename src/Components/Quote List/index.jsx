...
const QuotesList = ({ saveQuote, handleUnsave }) => {
  ...
  return (
    <div className='w-full sm:w-11/12 pb-10 h-[90vh] sm:h-[80vh]  bg-pink-200 my-2 overflow-auto scrollbar-track-gray-100  rounded-3xl flex  flex-col gap-7 sm:gap-14'>
      <h1 className='text-3xl sm:text-4xl font-semibold bg-emerald-200 w-full sm:w-1/3 h-36 sm:h-44 sm:rounded-ee-full flex items-center justify-center sm:pl-10'>YOUR QUOTES</h1>
      {saveQuote.length
      ?
      <ul className='flex flex-col items-center w-full gap-6 '>
      {saveQuote?.map((item)=>(
        <li key={item?.id} className='bg-pink-50 w-2/3 p-4 shadow-xl h-[30vh] flex justify-between sm:even:mr-64 sm:odd:ml-64 flex-col'>
      ...
      :
      <div className='w-full h-[calc(100%-112px)] flex items-center flex-col gap-2 justify-center text-3xl sm:text-5xl'>
        No Saved Quotes
